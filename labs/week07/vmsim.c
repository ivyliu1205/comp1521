...
int physicalAddress (uint vAddr, char action)
{
	...
	if((nPages * PAGESIZE) <= vAddr) return -1;
	
	if (PageTable[pageNo].status.loaded != 0) {
	    if (action == 'W') {
	        PageTable[pageNo].status.modified = 1;
	    }
	    PageTable[pageNo].lastAccessed = clock;
	    pAddr = (PAGESIZE * PageTable[pageNo].frameNo) + offset;
	} else {
	    int find = 0;
	    int frameIndex = 0;
	    ...
	    if (find != 1) {
	        ...
	        for (uint i = 0; i < nPages; i++) {
	            if (PageTable[i].lastAccessed != -1) {
	                if (PageTable[i].lastAccessed <= least) {
	                    least = PageTable[i].lastAccessed; 
	                }
	            }
	        }
	        int index = -1;
	        for (uint j = 0; j < nPages; j++) {
	            if (PageTable[j].lastAccessed == least){
	                index = j;
	                break;
	            }
	        }
	        frameIndex = PageTable[index].frameNo;
	        if (PageTable[index].status.modified != 0) {
	            nSaves++;
	        }
	        PageTable[index].status.modified = 0;
	        PageTable[index].status.loaded = 0;
	    ...
	    nLoads++;
	    MemFrames[frameIndex] = pageNo;
	    if (action == 'W') {
	        PageTable[pageNo].status.modified = 1;
	    }
	    PageTable[pageNo].status.loaded = 1;
	    PageTable[pageNo].lastAccessed = clock;
	    PageTable[pageNo].frameNo = frameIndex;
	    pAddr = (PAGESIZE * PageTable[pageNo].frameNo) + offset;
	}
